...
class PaymentTotalWigets extends StatefulWidget {
  const PaymentTotalWigets({Key? key, required this.shopId}) : super(key: key);
  final String shopId;
  @override
  State<PaymentTotalWigets> createState() => _SalesTotalWigetsState();
}

class _SalesTotalWigetsState extends State<PaymentTotalWigets> {
  late DatabaseReference _data;
  double todaySalesTotal = 0;
  double yesterdaySalesTotal = 0;
...
// final yesterdayDate= yesterday.toString().split('')[0];
  ...
  child: Padding(
    ...
    child: Column(
      ...
      children: [
        Text('\$ $amount', style: TextStyle(fontSize: MediaQuery.of(context).size.width * 0.08, color: Colors.white)),
        SizedBox(height: 10),
        Text(
          title,
